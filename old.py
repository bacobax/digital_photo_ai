...
class FacePipelineItem:
    ...
    effective_target_height_mm: Optional[float] = None
...
class FaceFramingPipeline:
    ...
    def _scale_crops_with_constraints(self) -> None:
        ...
        for idx, item in enumerate(self.items, start=1):
            ...
            top_pad, bottom_pad, hair_mm_pre, forehead_mm_pre = self._solve_padding(
                crop_h,
                hair_span_px,
                forehead_span_px,
            )
            ...
            if top_pad or bottom_pad:
                pre_parts = []
                if hair_mm_pre is not None:
                    pre_parts.append(f"hair-chin {hair_mm_pre:.2f} mm")
                if forehead_mm_pre is not None:
                    pre_parts.append(f"forehead-chin {forehead_mm_pre:.2f} mm")
                note = " | ".join(pre_parts) if pre_parts else ""
                print(
                    f"  [Face {idx}] Added padding top={top_pad}px bottom={bottom_pad}px before scaling"
                    + (f" ({note})" if note else "")
                )
            ...
            # Adaptive target height MM: choose effective T so spans can satisfy constraints when possible
            hair_mm = None
            forehead_mm = None
            T_default = self.params.target_height_mm
            T_low = -float("inf")
            T_high = float("inf")
            alpha = None
            beta = None
            if offsets["chin"] is not None and offsets["hair"] is not None and scaled_h > 0:
                alpha = max(1e-6, (offsets["chin"] - offsets["hair"]) / scaled_h)
                # T <= max_hair / alpha
                T_high = min(T_high, self.params.max_hair_to_chin_mm / alpha)
            if offsets["chin"] is not None and offsets["forehead"] is not None and scaled_h > 0:
                beta = max(1e-6, (offsets["chin"] - offsets["forehead"]) / scaled_h)
                # T >= min_forehead / beta
                T_low = max(T_low, self.params.min_forehead_to_chin_mm / beta)
            print(f"effective max crop h interval (mm): [{T_low:.2f}, {T_high:.2f}]")
            # Choose effective target height within feasible interval, preferring the default
            if T_low <= T_default <= T_high:
                T_eff = T_default
            else:
                # If default is below required lower bound, raise to lower bound; if above upper bound, drop to upper bound
                # But never go below zero
                # If both bounds invalid (empty interval), pick the closer one to the default
                if T_low > T_high:
                    # pick nearest bound
                    if abs(T_default - T_low) <= abs(T_default - T_high):
                        T_eff = max(1e-3, T_low)
                    else:
                        T_eff = max(1e-3, T_high)
                else:
                    T_eff = max(1e-3, min(max(T_default, T_low), T_high))

            item.effective_target_height_mm = T_eff

            px_per_mm = scaled_h / T_eff
            if alpha is not None:
                hair_mm = (offsets["chin"] - offsets["hair"]) / px_per_mm
            if beta is not None:
                forehead_mm = (offsets["chin"] - offsets["forehead"]) / px_per_mm
            ...
            if self.save_debug:
                ...
                if item.effective_target_height_mm is not None and abs(item.effective_target_height_mm - self.params.target_height_mm) > 1e-3:
                    metrics.append(f"targetH {item.effective_target_height_mm:.2f} mm")
                ...
                if item.effective_target_height_mm is not None and abs(item.effective_target_height_mm - self.params.target_height_mm) > 1e-3:
                    summary.append(f"targetH {item.effective_target_height_mm:.2f} mm")
    ...
    def _estimate_mm(self,
                     crop_h: float,
                     hair_span: Optional[float],
                     forehead_span: Optional[float],
                     top_pad: int,
                     bottom_pad: int) -> Tuple[Optional[float], Optional[float]]:
        total_height = crop_h + top_pad + bottom_pad
        if total_height <= 0:
            return None, None
        hair_mm = None
        if hair_span is not None:
            hair_mm = hair_span / total_height * self.params.target_height_mm
        forehead_mm = None
        if forehead_span is not None:
            forehead_mm = forehead_span / total_height * self.params.target_height_mm
        return hair_mm, forehead_mm

    def _solve_padding(self,
                       crop_h: float,
                       hair_span: Optional[float],
                       forehead_span: Optional[float]) -> Tuple[int, int, Optional[float], Optional[float]]:
        # Required total height lower bound from hair constraint
        th_min = None
        if hair_span is not None and self.params.max_hair_to_chin_mm > 0:
            th_min = (hair_span * self.params.target_height_mm) / self.params.max_hair_to_chin_mm

        # Allowed total height upper bound from forehead constraint
        th_max = None
        if forehead_span is not None and self.params.min_forehead_to_chin_mm > 0:
            th_max = (forehead_span * self.params.target_height_mm) / self.params.min_forehead_to_chin_mm

        current_h = crop_h
        target_h = current_h

        # Enforce lower bound (we can only add padding, not shrink)
        if th_min is not None:
            target_h = max(target_h, th_min)

        # If upper bound exists and is below what we can achieve (we can't shrink), prefer no padding
        if th_max is not None and target_h > th_max + 1e-6:
            target_h = current_h  # best effort; will warn later

        pad_total = int(max(0, math.ceil(target_h - current_h)))
        pad_total = min(pad_total, self.params.max_extra_padding_px)
        top_pad = pad_total // 2
        bottom_pad = pad_total - top_pad

        hair_mm, forehead_mm = self._estimate_mm(crop_h, hair_span, forehead_span, top_pad, bottom_pad)
        return top_pad, bottom_pad, hair_mm, forehead_mm
