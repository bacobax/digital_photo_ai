...
USAGE_BOTH = " (used by both closed-form and legacy pipelines)"
USAGE_CLOSED = " (used only by the closed-form pipeline)"
USAGE_LEGACY = " (used only by the legacy pipeline)"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Path to the input image{USAGE_BOTH}",
    )
    parser.add_argument(
        "--logdir",
        type=str,
        default="./logs",
        help=f"Directory where outputs/debug images are saved{USAGE_BOTH}",
    )
    parser.add_argument(
        "--top-margin-ratio",
        ...
        default=0.10,
        help=(
            "Fraction of the final box height kept above the hair line"
            f"{USAGE_LEGACY}"
        ),
    )
    parser.add_argument(
        ...
        default=0.80,
        help=(
            "Fraction of the final box height occupied before bottom extension"
            f"{USAGE_LEGACY}"
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=7.0 / 9.0,
        help=f"Target width/height ratio for adjusted boxes{USAGE_BOTH}",
    )
    parser.add_argument(
        "--min-width-px",
        dest="min_width_px",
        type=int,
        default=420,
        help=f"Minimum width of the final crop in pixels{USAGE_BOTH}",
    )
    parser.add_argument(
        "--min-height-px",
        dest="min_height_px",
        type=int,
        default=540,
        help=f"Minimum height of the final crop in pixels{USAGE_BOTH}",
    )
    parser.add_argument(
        "--resize-scaling",
        dest="resize_scaling",
        ...
        default=None,
        help=(
            "Optional scaling factor (0-1) applied to the pre-resize crop height/width"
            f"{USAGE_BOTH}"
        ),
    )
    parser.add_argument(
        ...
        default=36.0,
        help=(
            "Maximum allowed crown-to-chin distance in millimetres"
            f"{USAGE_BOTH}"
        ),
    )
    parser.add_argument(
        ...
        default=31.0,
        help=(
            "Minimum required crown-to-chin distance in millimetres"
            f"{USAGE_BOTH}"
        ),
    )
    parser.add_argument(
        ...
        default=34.0,
        help=(
            "Target crown-to-chin distance in millimetres"
            f"{USAGE_BOTH}"
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=45.0,
        help=f"Reference portrait height in millimetres{USAGE_BOTH}",
    )
    parser.add_argument(
        "--min-top-mm",
        dest="min_top_mm",
        ...
        default=4.0,
        help=(
            "Minimum clearance above the crown in millimetres"
            f"{USAGE_CLOSED}"
        ),
    )
    parser.add_argument(
        ...
        default=8.0,
        help=(
            "Minimum clearance below the chin in millimetres"
            f"{USAGE_CLOSED}"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Use the closed-form mm-budget solver for framing"
            f"{USAGE_CLOSED}"
        ),
    )
    parser.add_argument(
        ...
        action="store_false",
        help=(
            "Fallback to the legacy multi-stage padding pipeline"
            f"{USAGE_LEGACY}"
        ),
    )
    parser.add_argument(
        ...
        dest="no_debug",
        action="store_true",
        help=f"Disable saving debug images to disk{USAGE_BOTH}",
    )
